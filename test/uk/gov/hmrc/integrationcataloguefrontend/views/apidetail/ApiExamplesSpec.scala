...
class ApiExamplesSpec extends CommonViewSpec with ApiTestData {
  ...
  "ApiExampleView" should {
    ...
    "render page with api details" in new Setup {
      ...
      val page: Html         =
        apiExamples.render("title", List(Example("example response name 1", "example response body 1"), Example("example response name 2", "example response body 2")), idPrefix)
