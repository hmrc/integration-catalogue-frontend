...
class EmailConnectorISpec extends ServerBaseISpec with ApiTestData with BeforeAndAfterEach {
  ...
  "sendEmailToPlatform" should {
    ...
    "return true when the server response with accepted 202 status" in new Setup {
      ...
      val result: Boolean = await(emailConnector.sendEmailToPlatform(apiEmails, emailParams))
  ...
  "sendConfirmationEmailToSender" should {

    "return true when the server response with accepted 202 status" in new Setup {
      sendEmailStubWithStatus(ACCEPTED)

      val result: Boolean = await(emailConnector.sendConfirmationEmailToSender(senderEmail, emailParams))
